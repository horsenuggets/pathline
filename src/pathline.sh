#!/usr/bin/env zsh
# pathline - Git-aware path display with worktree highlighting.
# Source this file and call pathline_render to get colored path output.
...
# Main render function. Outputs the path with ANSI color codes, highlighting
# verified worktree names and appending the git branch when appropriate.
pathline_render() {
  local custom_path=$(pathline_get_path)
  local git_branch=$(pathline_get_branch)
  ...
  while [[ "$remaining" == *"$marker"* ]]; do
    ...
    for part in "${parts[@]}"; do
      ...
      local wt_branch=$(cd "$raw_candidate" 2>/dev/null && git symbolic-ref --short HEAD 2>/dev/null)
